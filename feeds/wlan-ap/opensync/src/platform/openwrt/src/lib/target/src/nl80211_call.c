...
static int nl80211_scan_dump_recv(struct nl_msg *msg, void *arg)
{
	...
	memset(tb, 0, sizeof(tb));
	nla_parse(tb, NL80211_BSS_MAX, genlmsg_attrdata(gnlh, 0),
		  genlmsg_attrlen(gnlh, 0), bss_policy);
	...
	if (tb[NL80211_BSS_TSF])
		neighbor->entry.tsf = nla_get_u64(tb[NL80211_BSS_TSF]);
	if (tb[NL80211_BSS_FREQUENCY])
		neighbor->entry.chan =
			ieee80211_frequency_to_channel((int)nla_get_u32(tb[NL80211_BSS_FREQUENCY]));
	if (tb[NL80211_BSS_SIGNAL_MBM])
		neighbor->entry.sig = ((int) nla_get_u32(tb[NL80211_BSS_SIGNAL_MBM])) / 100;
	if (tb[NL80211_BSS_SEEN_MS_AGO])
		neighbor->entry.lastseen = nla_get_u32(tb[NL80211_BSS_SEEN_MS_AGO]);
	if (tb[NL80211_BSS_BSSID])
		print_mac(neighbor->entry.bssid, nla_data(tb[NL80211_BSS_BSSID]));
	if (tb[NL80211_BSS_INFORMATION_ELEMENTS]) {
		int ielen = nla_len(tb[NL80211_BSS_INFORMATION_ELEMENTS]);
		unsigned char *ie = nla_data(tb[NL80211_BSS_INFORMATION_ELEMENTS]);
		int len;
			 ...
			 target_scan_cb_t *scan_cb, void *scan_ctx)
{
	...
	for (i = 0; i < chan_num; i ++) {
		syslog(0, "blogic %s:%s[%d]scan %d %d\n", __FILE__, __func__, __LINE__, i, ieee80211_channel_to_frequency(chan_list[i]));
